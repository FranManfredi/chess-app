...
public class CommonRule {
    public static Boolean checkRule(Board board, Piece piece, Coordinate toSquare) {
        ...
        return isNotSameColour(board, piece, toSquare);
    }

    private static Boolean isNotSameColour(Board board, Piece piece, Coordinate toSquare) {
        return Objects.equals(board.getSquare(toSquare).getPiece().getName(), "null") ||
                (!Objects.equals(board.getSquare(toSquare).getPiece().getColor(), piece.getColor()));
    }
}
