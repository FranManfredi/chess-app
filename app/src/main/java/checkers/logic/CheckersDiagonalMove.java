...
public class CheckersDiagonalMove implements Move {
    ...
    public CheckersDiagonalMove(int rowsIncremented, int columnIncremented, boolean backward) {
        this.rowsIncremented = rowsIncremented;
        this.columnIncremented = columnIncremented;
        this.backward = backward;
    }
    public CheckersDiagonalMove() {
    ...
    public CheckResult<Coordinate,Boolean> checkMove(Coordinate initialSquare, Coordinate finalSquare, Board board, SideColor color) {
        ...
        if (isNotMovingSameAmountHorizontalAndVertical(initialSquare, finalSquare))
            ...
            return new CheckResult<>(finalSquare, false,"Diagonal Movement Failed");
        } else if (isCollidingWithPiece(initialSquare, finalSquare, board)) {
            return new CheckResult<>(finalSquare, false,"Diagonal Movement Failed");
        } else
        ...
        if(isDiagonalClear(board,initialSquare, finalSquare))
            if (isMovingCorrectAmount(initialSquare, finalSquare)){
    ...
    private boolean isCollidingWithPiece(Coordinate initialSquare, Coordinate finalSquare, Board board) {
        return board.checkForPieceInSquare(finalSquare);
    }

    private boolean isMovingCorrectAmount(Coordinate initialSquare, Coordinate finalSquare) {
        return finalSquare.column() == initialSquare.column() + columnTemp && finalSquare.row() == initialSquare.row() + rowTemp;
    }

    private static boolean isNotMovingSameAmountHorizontalAndVertical(Coordinate initialSquare, Coordinate finalSquare) {
